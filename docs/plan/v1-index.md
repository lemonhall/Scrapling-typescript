...
| REQ-0001-006 | `v1-adaptive-fetchers` | `packages/core/src/__tests__/fetcher-base.test.ts` + `packages/node/src/__tests__/fetcher-static.test.ts` + `packages/webext/src/__tests__/fetcher-static.test.ts` | 2026-03-08 static fetcher baseline `13/13` passed（BaseFetcher config / Response contract / Node+WebExt GET+POST baseline） | doing |
...
- 当前已完成 foundation、parser baseline，并已进入 adaptive + static fetcher baseline：已支持内存快照、组合选择器拆分、显式 `identifier`、误命中保护、可注入 adaptive storage（core memory / node file / web storage），以及 `BaseFetcher/Response/Fetcher/AsyncFetcher` 的最小静态请求闭环；sessions、dynamic/stealth、proxy、spiders、CLI、MCP 仍待继续推进
