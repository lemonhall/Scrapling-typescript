...
- 关键 fixture 上，XPath、文本、导航、属性与文本 handler 断言全部通过，且 `Selector.text` 具备 handler 风格正则语义，collection 具备 `last/search/filter/get/getall` parity，并兼容 source-style snake_case aliases、selector utility helpers、`::attr(name)`、`find/find_all`、XPath variables、CSS `:contains()` 族语义与 comments/CDATA keep options
...
- `packages/core/src/__tests__/parser-advanced.test.ts` 中 `11/11` 通过
